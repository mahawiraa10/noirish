...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>{{ config('app.name', 'Noirish') }} - Register</title>
    ...
    <body class="font-sans text-gray-900 antialiased">
                    ...
                    <div class="max-w-md w-full">
                        ...
                        <form method="POST" action="{{ route('register') }}">
                            ...
                            <div>
                                ...
                                <x-text-input id="name" class="block mt-1 w-full custom-input-field" type="text" name="name" :value="old('name')" required autofocus autocomplete="name" />
                                <x-input-error :messages="$errors->get('name')" class_name="mt-2" />
                            </div>

                            <div class="mt-4">
                                <x-input-label for="email" :value="__('Email Address')" class="!text-gray-700" />
                                <x-text-input id="email" class="block mt-1 w-full custom-input-field" type="email" name="email" :value="old('email')" required autocomplete="username" />
                                <x-input-error :messages="$errors->get('email')" class_name="mt-2" />
                            </div>
                        ...
                        <p class="text-center text-sm text-gray-600 mt-6">
                            ...
                            <a href="{{ route('login') }}" class="underline text-slate-600 hover:text-slate-800">Log in here</a>
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div class="w-full text-center text-sm text-gray-500 py-4">
